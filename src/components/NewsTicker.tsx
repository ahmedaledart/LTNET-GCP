  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        links.forEach(link => {
          // Extract text carefully, prioritizing headings inside links
          const heading = link.querySelector('h1, h2, h3, h4, h5, h6');
          let titleText = heading ? heading.textContent : link.textContent;
          
          // Clean up text: remove newlines, extra spaces, and visually hidden text like "اقرأ المزيد"
          titleText = (titleText || '').replace(/\s+/g, ' ').trim();
          
          const href = link.getAttribute('href');
          
          // Filter criteria:
          // 1. Must have a title longer than 15 characters
          // 2. Must have an href
          // 3. Href must be an article link (usually contains /arabic/ or /articles/)
          // 4. Exclude topic pages and generic links
          if (
            titleText && 
            titleText.length > 15 && 
            href && 
            (href.includes('/arabic/') || href.includes('/articles/')) &&
            !href.includes('/topics/') &&
            !titleText.includes('اقرأ المزيد')
          ) {
            const fullLink = href.startsWith('http') ? href : `https://www.bbc.com${href.startsWith('/') ? '' : '/'}${href}`;
            
            // Deduplicate by both title and link
            if (!seenTitles.has(titleText) && !seenLinks.has(fullLink)) {
        ...
        if (extractedNews.length > 0) {
          ...
          throw new Error("No news found in the parsed HTML");
        }
      } catch (error) {
